...
function searchStringTokensRegExGenerator (searchQuery, flags) {
    ...
    if (words.length >= 2) {
        ...
        // use simple search when there is only one word, such that it can match any part of the word.
        // eg. a search query of "ere" will match and string contain "ere"
        var onlyWord = words[0];
        if (debug) console.log('onlyWord: ', onlyWord);
        generatedRegEx = searchQuery.replace(_regexObj_reEscape, '\\$1' );
